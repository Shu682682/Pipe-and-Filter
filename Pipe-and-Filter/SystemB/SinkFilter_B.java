...
public class SinkFilter_B extends FilterFramework_B {
		...
		while (true) {
			try {
				...
				if ( id == 4 )
				{
					...
					String strAltitude = Double.toString(altitude);
					//If wildJump happened add *
					if(wildJump == (byte) 1){
						strAltitude+='*';
					}
					wildJump=(byte)0;
					String data = TimeStampFormat.format(TimeStamp.getTime()) + ',' + velocity + ',' + strAltitude + ',' + pressure + ',' + temperature + '\n';
					list.add(data);
				}
	...
	public static void writeToCSV(List<String> list) throws IOException {
		FileWriter writer = new FileWriter("outputB(Test).csv");
